...
function content_grid_append_to_content( $content ) {
	...
	if( is_page() ) {
		...
		$append_html .= $rn.'<!-- powered by Content-Grid plugin v.'.CONTENT_GRID_PLUGIN_VERSION.' wordpress.org/plugins/content-grid/ -->'.$rn;
...
function content_grid_enqueue_assets() {
	if (is_page()) { // load assets only on pages
		wp_enqueue_style('content-grid-style', plugins_url('/css/content-grid.css', __FILE__), false, CONTENT_GRID_PLUGIN_VERSION, 'all');
	}
}
