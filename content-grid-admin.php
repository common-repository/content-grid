...
function content_grid_schema( $post_id ) {
	...
	$content_grid_row_options = [
		0 => 'empty',
		1 => __( '1 column (100%)', 'content-grid' ),
		2 => __( '2 columns (50%/50%)', 'content-grid' )
	];
	...
	for ( $i_row = 1; $i_row <= $content_grid_settings['content_rows']; $i_row++ ): // run thru rows
		...
		endforeach;
		$html .= '</select>';
		$html .= '<div class="cg-pull-right">';
		$html .= sprintf( __( 'row #%s', 'content-grid' ), $i_row );
		$html .= '</div>';
		$html .= '</div><!-- .cg-col-12 -->';
		...
		for ($i_col = 1; $i_col <= 2; $i_col++): // run thru cols
			...
			$html .= __( 'Content', 'content-grid' );
			$html .= ' <select class="cg-col-control" name="content_grid_col_row_'.$i_row.'_col_'.$i_col.'">';
			...
			$html .= '<div class="cg-pull-right">';
			$html .= sprintf( __( 'col #%s', 'content-grid' ), $i_col );
			$html .= '</div>';
	...
	$html .= '<p>'.__( 'Do not forget to click the Update button at the Publish section to save all your changes.', 'content-grid' ).'</p>';
...
function content_grid_meta_box_callback( $post ) {
	...
	echo '<a href="#" class="nav-tab content-grid-nav-tab content-grid-nav-tab-1">'.__( 'Grid', 'content-grid' ).'</a>';
...
function content_grid_admin_enqueue_assets() {
	wp_enqueue_style( 'content-grid-admin-style', plugins_url( '/css/content-grid-admin.css', __FILE__ ), false, CONTENT_GRID_PLUGIN_VERSION, 'all' );
	wp_enqueue_script( 'content-grid-admin-script', plugins_url( '/js/content-grid-admin.js', __FILE__ ), array('jquery'), CONTENT_GRID_PLUGIN_VERSION );
}
